...
#import "RNWebrtcPushNotification.h"
...
@implementation RNWebrtcPushNotification
...
- (void)dealloc
{
    ...
    for (NSString *uuid in [RNWebrtcPushNotification completionHandlers]) {
        RNVoipPushNotificationCompletion completion = [[RNWebrtcPushNotification completionHandlers] objectForKey:uuid];
        if (completion) {
    ...
    [[RNWebrtcPushNotification completionHandlers] removeAllObjects];
}
...
- (NSDictionary<NSString *, id> *)constantsToExport
{
    ...
    NSLog(@"[RNWebrtcPushNotification] constantsToExport currentState = %@", currentState);
...
{
    NSLog(@"[RNWebrtcPushNotification] voipRegistration");
    ...
    dispatch_async(mainQueue, ^{
      ...
      voipRegistry.delegate = (RNWebrtcPushNotification *)RCTSharedApplication().delegate;
...
{
    NSLog(@"[RNWebrtcPushNotification] didUpdatePushCredentials credentials.token = %@, type = %@", credentials.token, type);
...
{
    NSLog(@"[RNWebrtcPushNotification] didReceiveIncomingPushWithPayload payload.dictionaryPayload = %@, type = %@", payload.dictionaryPayload, type);
...
{
    NSLog(@"[RNWebrtcPushNotification] handleRemoteNotificationsRegistered notification.userInfo = %@", notification.userInfo);
...
{
    NSLog(@"[RNWebrtcPushNotification] handleLocalNotificationReceived notification.userInfo = %@", notification.userInfo);
...
{
    NSLog(@"[RNWebrtcPushNotification] handleRemoteNotificationReceived notification.userInfo = %@", notification.userInfo);
...
RCT_EXPORT_METHOD(onVoipNotificationCompleted:(NSString *)uuid)
{
    RNVoipPushNotificationCompletion completion = [[RNWebrtcPushNotification completionHandlers] objectForKey:uuid];
    if (completion) {
        [RNWebrtcPushNotification removeCompletionHandler: uuid];
        dispatch_async(dispatch_get_main_queue(), ^{
            NSLog(@"[RNWebrtcPushNotification] onVoipNotificationCompleted() complete(). uuid = %@", uuid);
            completion();
        });
    } else {
        NSLog(@"[RNWebrtcPushNotification] onVoipNotificationCompleted() not found. uuid = %@", uuid);
    }
}
